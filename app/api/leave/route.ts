...
function serialize(request: Awaited<ReturnType<typeof prisma.leaveRequest.findMany>>[number] & { personnel?: { serviceNumber: string; fullName: string; rank: string } }) {
	...
	return { ...request, amendment: amendment ? { ...amendment, from: amendment.fromDate.slice(0, 10), to: amendment.toDate.slice(0, 10), reportingDate: amendment.reportingDate.slice(0, 10) } : null, armyNo: request.personnel?.serviceNumber, name: request.personnel?.fullName, rank: request.personnel?.rank, type: request.leaveType, from: request.fromDate.toISOString().slice(0, 10), to: request.toDate.toISOString().slice(0, 10), prefix: request.prefixDate?.toISOString().slice(0, 10) ?? "None", suffix: request.suffixDate?.toISOString().slice(0, 10) ?? "None", reportingDate: request.reportingDate.toISOString().slice(0, 10), status: request.status === "REJECTED" ? "Rejected" : request.status === "APPROVED" || request.status === "SDM_APPROVED" || request.status === "ADJT_APPROVED" ? "Approved" : "Pending" };
}

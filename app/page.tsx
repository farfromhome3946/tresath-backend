...
const SQUADRON_THEME: Record<string, { community: string | null; accent: string; Icon: (props: { size?: number }) => ReactNode; gif?: string }> = {
  AGNI: { community: "Rajput", accent: "#7a1f2b", Icon: RajputCrestIcon, gif: "/gifs/rajputs.webp" },
  BHARAT: { community: "Jat", accent: "#1d6047", Icon: JatCrestIcon, gif: "/gifs/jat.webp" },
  CHARDIKALA: { community: "Sikh", accent: "#0b2e57", Icon: KhandaIcon, gif: "/gifs/sikh.webp" },
...
function Auth({ register, setRegister, onLogin }: { register: boolean; setRegister: (value: boolean) => void; onLogin: (account: Account) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(""); try { const response = await fetch(api("/api/personnel"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, action: register ? "register" : "login" }) }); const data = await response.json(); if (!response.ok) { if (!register) setShowWrongPassword(true); throw new Error(data.error ?? "Authentication failed."); } onLogin(data.account); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to connect to Tresath."); } finally { setBusy(false); } }
  return (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-6" onClick={() => setShowWrongPassword(false)}>
          <div className="max-w-xs text-center" onClick={(event) => event.stopPropagation()}>
            <div className="relative overflow-hidden rounded-2xl border border-[#d7ae3d]/40 shadow-2xl">
              ...
              <p className="pointer-events-none absolute inset-x-0 bottom-[4%] text-sm font-bold uppercase tracking-wide text-white [text-shadow:1px_1px_0_#000,-1px_-1px_0_#000,1px_-1px_0_#000,-1px_1px_0_#000]">Chal Jhoote</p>
            </div>
...
function Overview({ account, leave, people, tdy }: { account: Account; leave: Leave[]; people: Person[]; tdy: TDY[] }) {
  const [selectedSquadron, setSelectedSquadron] = useState<string | null>(null);
  const today = calendarDate();
  const tomorrowDate = new Date();
  tomorrowDate.setDate(tomorrowDate.getDate() + 1);
  const tomorrow = calendarDate(tomorrowDate);
  const activeSquadron = account.role === "ADJT" ? selectedSquadron : account.squadron;
  const scopedLeave = activeSquadron ? leave.filter((item) => item.squadron === activeSquadron) : leave;
  const scopedTdy = activeSquadron ? tdy.filter((item) => item.squadron === activeSquadron) : tdy;
  const scopedPeopleCount = activeSquadron ? people.filter((person) => person.squadron === activeSquadron).length : people.length;
  const onLeaveRecords = scopedLeave.filter((item) => item.status === "Approved" && item.from <= today && item.reportingDate > today);
  const onTdyRecords = scopedTdy.filter((item) => item.from <= today && item.to >= today);
